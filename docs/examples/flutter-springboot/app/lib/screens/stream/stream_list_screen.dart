...
class _StreamListScreenState extends State<StreamListScreen> {
  ...
  Future<List<Map<String, dynamic>>> _loadStreams() {
    return context.read<ApiService>().getStreams();
  }
        ...
        builder: (context, snapshot) {
          ...
          return Scaffold(
            ...
            floatingActionButton: FloatingActionButton.extended(
              onPressed: () async {
                final result = await Navigator.of(context).pushNamed('/stream/host');
...
class _StreamCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hostName = stream['hostName'] as String? ?? stream['host_name'] as String? ?? 'Unknown';
    final viewerCount = stream['viewerCount'] as int? ?? stream['viewer_count'] as int? ?? 0;
    final isLive = stream['status'] == 'live' || stream['isLive'] == true;
