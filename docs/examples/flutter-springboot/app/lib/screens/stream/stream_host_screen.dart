...
class _StreamHostScreenState extends State<StreamHostScreen> {
  ...
  Future<void> _goLive() async {
    ...
    try {
      ...
      final result = await api.createStream(title: title);
      final streamId = result['id'] as String? ?? result['streamId'] as String? ?? '';
      ...
      if (mounted) {
        ...
        Timer.periodic(const Duration(seconds: 5), (timer) {
          if (!mounted || _state != _HostState.live) {
            timer.cancel();
            return;
          }
          // In production, this would come from a WebSocket event
        });
      }
    } catch (e) {
      if (mounted) {
