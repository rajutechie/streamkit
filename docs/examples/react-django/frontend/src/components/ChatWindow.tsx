 ...
 * real-time messages, send messages, handle typing indicators, and
 * support reactions and read receipts.
 */
...
const REACTION_EMOJIS = ['\u{1F44D}', '\u{2764}\u{FE0F}', '\u{1F602}', '\u{1F622}', '\u{1F64F}', '\u{1F525}'];
...
function MessageBubble({ message, isMine, presenceMap, currentUserId }: MessageBubbleProps) {
  ...
  // Read receipts -- show a double-check if anyone else has read
      ...
      <div className="message-bubble-wrapper">
        ...
        {Object.keys(reactions).length > 0 && (
          <div className="message-reactions">
            {Object.entries(reactions).map(([emoji, userIds]) => (
              <span
                key={emoji}
                className={`reaction-chip ${(userIds as string[]).includes(currentUserId) ? 'mine' : ''}`}
                title={(userIds as string[]).join(', ')}
              >
        ...
        <div className="reaction-bar">
          {REACTION_EMOJIS.map((emoji) => (
            <button key={emoji} className="reaction-bar-btn" title={`React with ${emoji}`}>
              {emoji}
            </button>
          ))}
        </div>
