...
export default function ChannelList({
  ...
  function channelDisplayName(ch: ChannelData): string {
    if (ch.type === 'group') return ch.name || 'Unnamed Group';
    // For DMs, show the other member's ID (in a real app you'd resolve names)
    const other = ch.members?.find((m) => m !== currentUserId);
    return other ?? 'Direct Message';
  }

  function isOnline(ch: ChannelData): boolean {
  ...
  const renderSection = (title: string, items: ChannelData[]) => {
