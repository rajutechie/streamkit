...
class _StartCallScreenState extends State<StartCallScreen> {
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      ...
      final users = await auth.api.getUsers();
      if (mounted) {
        setState(() {
  ...
  Future<void> _startCall() async {
    ...
    try {
      final auth = context.read<AuthService>();
      final result = await auth.api.createCall(
        type: _callType,
        participantIds: [_selectedUserId!],
      );

      if (mounted) {
        final callId = result['id'] as String? ?? result['call_id'] as String? ?? '';
