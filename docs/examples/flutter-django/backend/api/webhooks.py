...
``rajutechie_streamkit_django`` matches events to the right function, and ``webhook_view``
(wired in ``api/urls.py``) verifies the request signature before dispatching.
"""
...
def handle_new_message(event):
    ...
    Use this to trigger push notifications, store analytics,
...
def handle_call_ended(event):
    ...
    Use this to log call duration, update billing, send a summary, etc.
    """
    data = event.data
    logger.info(
...
def handle_stream_started(event):
    ...
    Use this to notify subscribers, kick off recording, or update a dashboard.
    """
    data = event.data
    logger.info(
...
def handle_stream_ended(event):
    ...
    Use this to finalize recording, compute analytics, archive the VOD, etc.
    """
    data = event.data
    logger.info(
...
def handle_meeting_started(event):
    ...
    Use this to notify remaining invitees, start recording, etc.
    """
    data = event.data
    logger.info(
...
def handle_meeting_ended(event):
    ...
    Use this to finalize recordings, send summaries, update billing, etc.
    """
    data = event.data
    logger.info(
